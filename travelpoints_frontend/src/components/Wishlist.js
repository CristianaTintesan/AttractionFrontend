...
import {deleteWishlist, getUser, getWishlist, insertWishlist} from "../services/wishlistService";
...
const Wishlist = () => {
    ...
    const handleRerender = async () => {
        let userWishlits = await getWishlist();
        let data2 = [];
        userWishlits = userWishlits.filter((elem)  => (Number(elem.user.id) === Number(userId)) );
        console.log(userWishlits);
        for (const x of userWishlits){
            console.log(x);
            data2.push(x.attraction);
        }
